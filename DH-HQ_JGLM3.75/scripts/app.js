...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
  ...
  `;
            ...
            if (statsBtn) {
                ...
                // Always fetch the latest selection when clicked
                statsBtn.onclick = () => showPlayerComparison(getSelectedTradePlayers());
            }
        ...
        async function showPlayerComparison(players) {
            if (players.length < 2) {
                // Provide user feedback when not enough players are selected
                compareModalBody.innerHTML = '<p class="text-center p-4">Select at least two players to compare.</p>';
                openCompareModal();
                return;
            }
