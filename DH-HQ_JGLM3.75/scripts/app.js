...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
        ...
        if (pageType === 'rosters') {
            ...
            tradeSimulator?.addEventListener('click', handleTradeSimulatorClick);
    ...
    hideLegend();
        ...
        function handleTradeSimulatorClick(e) {
            const statsBtn = e.target.closest('#tradeCompareStats');
            if (statsBtn) {
                const current = getSelectedTradePlayers();
                if (current.length >= 2) {
                    showPlayerComparison(current);
                }
            }
        }
  ...
  `;
        ...
        function updateTradeStatsButton() {
            const btn = document.getElementById('tradeCompareStats');
            if (btn) {
                btn.disabled = getSelectedTradePlayers().length < 2;
            }
        }
