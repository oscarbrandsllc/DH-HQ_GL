...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        function renderPlayerComparison(players) {
            ...
            headerRow.appendChild(document.createElement('div')); // Empty cell for alignment
            players.forEach(player => {
                const playerHeader = document.createElement('div');
                playerHeader.className = 'player-header';
                const fullPlayer = state.players[player.id];
                const playerName = fullPlayer ? `${fullPlayer.first_name} ${fullPlayer.last_name}` : player.name;
                playerHeader.innerHTML = `<h4>${playerName}</h4><span class="player-pos-team">${player.pos} - ${fullPlayer.team || 'FA'}</span>`;
            ...
            for (const statKey of allStatKeys) {
                if (statLabels[statKey]) {
                    ...
                    for (let i = 0; i < players.length; i++) {
                        ...
                        const avg = player.gameLogs.length > 0 ? total / player.gameLogs.length : 0;
                        values.push(avg);
                        if (avg > maxVal) {
                            maxVal = avg;
                            maxIndex = i;
  ...
  `;
            ...
            document.getElementById('comparePlayersButton').addEventListener('click', handlePlayerCompare);
