...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
            ...
            summaryChipsContainer.innerHTML = `
                ...
                <div class="summary-chip">
                    ...
                    <div class="chip-values">
                        ...
                        <span class="chip-separator">/</span>
                        <span>
                            <span style="color: ${getPosLabelColor(player.pos)}">${player.pos}&middot;</span>
                            <span style="color: ${getPosRankValueColor(player.pos, playerRanks.posRank)}">${formatPos(playerRanks.posRank)}</span>
                        </span>
                    </div>
                </div>
                <div class="summary-chip">
                    ...
                    <div class="chip-values">
                        ...
                        <span>
                            <span style="color: ${getPosLabelColor(player.pos)}">${player.pos}&middot;</span>
                            <span style="color: ${getPosRankValueColor(player.pos, playerRanks.ppgPosRank)}">${formatPos(playerRanks.ppgPosRank)}</span>
                        </span>
                    </div>
                </div>
            `;
  ...
  `;
    ...
    hideLegend();
        ...
        function getPosLabelColor(pos) {
            const colors = { QB: '#FF7AB2', RB: '#bbf7e0', WR: '#A0C2F7', TE: '#ffae58' };
            return colors[pos] || 'var(--color-text-secondary)';
        }

        function getPosRankValueColor(pos, rank) {
            if (typeof rank !== 'number') return 'var(--color-text-primary)';
            const wrThresholds = [
                { v: 6, c: '#00ffc4' },
                { v: 12, c: '#85fff3' },
