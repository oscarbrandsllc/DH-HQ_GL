...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        async function handlePlayerCompare(e) {
            ...
            if (selectedPlayersWithTeams.length !== 2) {
                // The button should be disabled, but as a fallback, do nothing.
                return;
            }
  ...
  `;
            ...
            if (comparePlayersButton) {
                ...
                if (playerCount !== 2) {
                    comparePlayersButton.disabled = true;
                    comparePlayersButton.setAttribute('title', 'Please select exactly 2 players to compare.');
                } else {
                    comparePlayersButton.disabled = false;
                    comparePlayersButton.removeAttribute('title');
                }
            }
